...
namespace Mud2D.models
{
    public class Map
    {
        ...
        public Map(string mapFilename)
        {
            ...
            Monsters.Add(new Monster() { Name = "Ogre", Life = 5, X=blank.X, Y=blank.Y, LastY=blank.X, LastX=blank.Y });
...
//            ThePlayer.Erase();  //clear the space where the player is moving from
            switch (dir)
            {
...
//            ThePlayer.Update();   //must redraw the player since we aren't redrawing the entire map every time
        }
