...
namespace Mud2D.models
{
    ...
    {
        [JsonIgnore]        //no need to serialize the dirty flag
        ...
        /// <summary>
        /// Find the most damage can deal from items in inventory
        /// </summary>
        /// <returns></returns>
        internal int CanDealDamage()
        {
            int maxdamage = 1; //bare hands
            foreach(var i in Inventory)
            {
                if(i.Attrib.ContainsKey("Damage"))
                {
                    int dam = Convert.ToInt16(i.Attrib["Damage"]);
                    if (dam > maxdamage) maxdamage = dam;
                }
            }
            return maxdamage;
        }
        ...
        {
            Life = Math.Min(Life + value, MaxLife);    //we can't go past maxlife so heal up to that as max.
        }
    }
}
